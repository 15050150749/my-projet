...
from dict_format import dict_format, change_dict_key_pattern, exclude_str_root
...
class BaseAssert(object):
    ...
    @classmethod
    def assert_dic_item_type(cls, expect, actual, error_msg=""):
        for k in actual.keys():
            v = actual[k]
            cls.assert_in(k, expect.keys())
            if "dict" in str(type(v)):
                cls.assert_dic_item_type(expect[k], actual[k])
            elif "list" in str(type(v)):
                if len(v) > 0:
                    for i in range(len(v)):
                        cls.assert_dic_item_type(expect[k][0], actual[k][i])
                else:
                    pass
            else:
                cls.assert_in(expect[k], str(type(v)), "数据类型不对")
